...
| `flutter test` | 44 pass / 3 fail | **46 pass / 1 fail** |
...
| 3 | Persistence_Store foundation (Req 6.6, 6.7) | in progress |
| 4 | App_Lock security surface (Req 5.1 to 5.5) | not started |
...
- `Persistence_Store` does not exist. No `shared_preferences`, no secure storage, so
  session, PIN, theme, masking and every mutation die on restart. Requirements 6.6, 6.7,
  1.14, 5.6, 5.10 and 9.5 all depend on it.
- `App_Lock` has no biometric path, no 5 attempt lockout, no 120 second background
  timeout, and does not obscure content in the task switcher.
